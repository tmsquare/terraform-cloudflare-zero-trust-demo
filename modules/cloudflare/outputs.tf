...
output "gcp_extracted_token" {
  ...
  value       = data.cloudflare_zero_trust_tunnel_cloudflared_token.gcp_tunnel_cloudflared_token.token
  sensitive   = "true"
}

output "aws_extracted_token" {
  description = "token destined to be used in the startup script of AWS VM"
  value       = data.cloudflare_zero_trust_tunnel_cloudflared_token.aws_tunnel_cloudflared_token.token
  sensitive   = "true"
}

output "gcp_windows_extracted_token" {
  description = "token destined to be used in the startup script of GCP Windows VM"
  value       = data.cloudflare_zero_trust_tunnel_cloudflared_token.gcp_tunnel_cloudflared_windows_rdp_token.token
  sensitive   = "true"
}
...
output "gcp_tunnel_id" {
  ...
  value       = cloudflare_zero_trust_tunnel_cloudflared.gcp_cloudflared_tunnel.id
  sensitive   = "true"
}

output "gcp_windows_rdp_tunnel_id" {
  description = "ID of the Cloudflare Zero Trust Tunnel to GCP for Windows RDP"
  value       = cloudflare_zero_trust_tunnel_cloudflared.gcp_cloudflared_windows_rdp_tunnel.id
  sensitive   = "true"
}

output "aws_tunnel_id" {
  description = "ID of the Cloudflare Zero Trust Tunnel to AWS"
  value       = cloudflare_zero_trust_tunnel_cloudflared.aws_cloudflared_tunnel.id
  sensitive   = "true"
}
...
output "gcp_tunnel_status" {
  ...
  value       = cloudflare_zero_trust_tunnel_cloudflared.gcp_cloudflared_tunnel.status
  depends_on  = [cloudflare_zero_trust_tunnel_cloudflared.gcp_cloudflared_tunnel]
}

output "gcp_windows_rdp_tunnel_status" {
  description = "GCP Tunnel Status for Windows RDP"
  value       = cloudflare_zero_trust_tunnel_cloudflared.gcp_cloudflared_windows_rdp_tunnel.status
  depends_on  = [cloudflare_zero_trust_tunnel_cloudflared.gcp_cloudflared_windows_rdp_tunnel]
}

output "aws_tunnel_status" {
  description = "AWS Tunnel Status"
  value       = cloudflare_zero_trust_tunnel_cloudflared.aws_cloudflared_tunnel.status
  depends_on  = [cloudflare_zero_trust_tunnel_cloudflared.aws_cloudflared_tunnel]
}
...
output "gcp_tunnel_version" {
  ...
  value       = length(cloudflare_zero_trust_tunnel_cloudflared.gcp_cloudflared_tunnel.connections) > 0 ? cloudflare_zero_trust_tunnel_cloudflared.gcp_cloudflared_tunnel.connections[0].client_version : "no connections yet"
  depends_on  = [cloudflare_zero_trust_tunnel_cloudflared.gcp_cloudflared_tunnel]
}

output "gcp_windows_rdp_tunnel_version" {
  description = "version of Cloudflared running on GCP Windows VM"
  value       = length(cloudflare_zero_trust_tunnel_cloudflared.gcp_cloudflared_windows_rdp_tunnel.connections) > 0 ? cloudflare_zero_trust_tunnel_cloudflared.gcp_cloudflared_windows_rdp_tunnel.connections[0].client_version : "no connections yet"
  depends_on  = [cloudflare_zero_trust_tunnel_cloudflared.gcp_cloudflared_tunnel]
}

output "aws_tunnel_version" {
  description = "version of Cloudflared running on AWS VM"
  value       = length(cloudflare_zero_trust_tunnel_cloudflared.aws_cloudflared_tunnel.connections) > 0 ? cloudflare_zero_trust_tunnel_cloudflared.aws_cloudflared_tunnel.connections[0].client_version : "no connections yet"
  depends_on  = [cloudflare_zero_trust_tunnel_cloudflared.aws_cloudflared_tunnel]
}
